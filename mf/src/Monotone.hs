...
bool :: ConstData -> Bool
bool (Bool x) = x
bool x = error $ "Called bool with wrong argument: " ++ show x
...
constTransfer (BAssign' l name val) (Just input) = Just $ Map.insert name (boolToConstData (evalB input val)) input
...
evalB :: Map String ConstData -> BExpr -> Maybe Bool
evalB input = eval
    where
    eval (BConst v) = Just v
    eval (BVar n) = bool <$> Map.lookup n input
    eval _ = undefined
