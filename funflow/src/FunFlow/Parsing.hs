...
pExpr :: Parser Expr
pExpr = (pFn <|> pFun <|> pITE <|> pLet <|> pPair <|> pPCase <|> pCons <|> pNil <|> pLCase <|> pDType <|> pDTCase) <<|> pBin
  where
  ...
  -- simple expressions
  pFn, pFun, pLet, pITE, pPair, pPCase, pCons, pNil, pLCase, pDType, pDTCase :: Parser Expr
