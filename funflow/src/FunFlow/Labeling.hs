...
label (Cons _ e1 e2) = Cons <$> fresh <*> label e1 <*> label e2
label (Nil _) = Nil <$> fresh
label (LCase e1 n1 n2 e2 e3) = LCase <$> label e1 <*> pure n1 <*> pure n2 <*> label e2 <*> label e3
label (DType _ es) = DType 0 <$> sequence (map label es)
label (DTCase e1 ns e2 e3) = DTCase <$> label e1 <*> pure ns <*> label e2 <*> label e3
